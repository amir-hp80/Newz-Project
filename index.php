...
<body class="animsition">
	...
	<section class="bg0 p-t-70">
        ...
        WHERE 
            categories.name IN ('Entertainment', 'Music', 'Game', 'Celebrity')
        ORDER BY 
            posts.created_at DESC
        LIMIT 4";

								$posts = db_query($query); // این تابع فرضی است و باید با تابع پایگاه داده شما جایگزین شود.

								if (!empty($posts)) {
									foreach ($posts as $index => $post) {
										if ($index === 0) {
											// نمایش پست اصلی (اولین پست)
								?>
											<div class="col-sm-6 p-r-25 p-r-15-sr991">
												<div class="m-b-30" onclick="window.location.href='pages/news_detail.php?id=<?php echo htmlspecialchars($post['id']); ?>'" style="cursor: pointer;">
													<a class="wrap-pic-w hov1 trans-03">
														<img src="pages/uploads/<?php echo htmlspecialchars($post['image']); ?>" alt="IMG">
													</a>
										...
										?>
											<div class="flex-wr-sb-s m-b-30" onclick="window.location.href='pages/news_detail.php?id=<?php echo htmlspecialchars($post['id']); ?>'" style="cursor: pointer;">
												<a class="size-w-1 wrap-pic-w hov1 trans-03">
													<img src="pages/uploads/<?php echo htmlspecialchars($post['image']); ?>" alt="<?php echo htmlspecialchars($post['title']); ?>">
												</a>
            ...
            ";
									...
									<div class="col-sm-6 p-r-25 p-r-15-sr991 p-b-25">
										<div class="how2 how2-cl2 flex-sb-c m-b-35">
											...
											<a href="pages/category.php?id=<?= htmlspecialchars($category['id']); ?>" class="tab01-link f1-s-1 cl9 hov-cl10 trans-03">
												View all
												<i class="fs-12 m-l-5 fa fa-caret-right"></i>
											</a>
										</div>
